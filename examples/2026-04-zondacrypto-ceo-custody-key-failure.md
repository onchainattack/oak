...
**OAK Techniques observed:** none — an operator key lost, withheld, or rendered unusable with the vector unestablished has no covering Technique. The file states the funds' status ("stolen, frozen, lost, or withheld by the previous CEO") was unknown, so no mechanism is established at all. Every T11 sub-Technique was checked: T11.001 (no external attacker, no vendor), T11.003 (there was no multisig — that is the finding), T11.005.x (the exchange was real and operating since 2014; it was not the fraud vehicle), T11.006 (no exfiltration established), T11.010 (no counterparty default), T11.012 (server-side key storage not described). T15 does not hold: an M&A transition in which keys were never handed over is not a compromise, and the organised-crime dimension is expressly alleged but not established. Candidate gap: single-individual custody concentration / key-succession failure at an operator. **Contrast with QuadrigaCX** (`examples/2019-01-quadrigacx.md`), which *does* carry sub-Techniques only because a monitor established commingling and diversion — the two look alike narratively and are taxonomically opposite: Quadriga has an established mechanism, Zondacrypto an established absence of one.
