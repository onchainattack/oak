...
**OAK Techniques observed:** **OAK-T5.005** (Treasury-Management Exit) — primary; the founder exercised his standing single-signature authority over the project treasury to move SOL to himself, then spent it on NFTs. T5.005 is dual-parented under T5 (Value Extraction) and T6 (Defense Evasion), which absorbs the fabricated-hack narrative: the "I was hacked" performance is the evasion layer over an authority-misuse extraction, not a separate access-acquisition event. **Partially unmapped:** no T6 sub-Technique covers an operator-fabricated hack claim (T6.001–T6.004 are audit/source-verification claims; T6.005–T6.008 are upgrade, counterfeit-token, vendor-promise and frontend surfaces). **Not T9** — the file establishes a single-signature founder wallet; there is no contract and no exploit. **Not T4** — no access was acquired; the founder already held it.
