...
**OAK Techniques observed:** **OAK-T8.001** (Common-Funder Cluster Reuse) and **OAK-T8.003** (On-Chain Transaction Graph De-Anonymization) — the attribution mechanism, and the only one the file establishes; the suspect's "lifestyle" NFT wallets were linkable to their identity through funding and behavioural patterns despite being separate from the hack's laundering addresses. **Not tagged:** the NFT round-tripping is described without establishing its *goal*, and goal is what separates T12.001 (price-discovery distortion to attract real buyers) from T7.004 (NFT wash-laundering). Tagging either would infer intent the file does not evidence. **No T9 sub-Technique:** the file states only that "Bittensor (TAO) suffered a \$28M hack" and never describes the exploit mechanism. **No T7 sub-Technique:** "laundered through standard paths" names no rail.
