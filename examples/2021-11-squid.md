...
**OAK Techniques observed:** OAK-T1.001 (Modifiable Tax / Anti-sell), **OAK-T2.001** (Single-Sided Liquidity Plant — the deployer seeded the SQUID side of the PancakeSwap pool with near-zero-cost tokens while buyers provided BNB; the single-sided provisioning structure concentrated all value-side liquidity in user deposits, enabling the subsequent drain), OAK-T5.001 (Hard LP Drain), OAK-T8.001 (Operator-Cluster Reuse).
**Status:** operator publicly identified as a recurring cluster; specific named individuals not publicly disclosed.

**Key teaching point:** SQUID is a useful didactic case because it bundles a static-detectable T1.001 pattern with a textbook T5.001 single-event drain, and because TRM's published cluster attribution makes it one of the few public examples where the OAK-T8.001 case is on the public record at the time of the incident rather than reconstructed years later. Contributors writing analogous post-2024 cases should note that more recent operator clusters tend to layer T6 (Defense Evasion) patterns — fragmented timing, multi-actor coordination — over the same underlying T1+T5 substrate.
