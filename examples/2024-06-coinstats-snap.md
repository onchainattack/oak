...
**OAK Techniques observed:** **OAK-T11.002** (Wallet-Software Distribution Compromise — MetaMask Snap distribution-channel subclass) + **OAK-T15.004** (Operator-Side Credential Compromise) — canonical anchor for the **operator-cloud / SaaS sub-shape**: the load-bearing compromise was at the CoinStats-controlled iOS push-notification infrastructure account / SaaS surface, used to distribute a malicious Snap to the operator's user base. + **OAK-T15.002** (Supply-Chain / Vendor-Pipeline Compromise) — the Snap-distribution channel is structurally a shared-upstream-surface attack class. + **OAK-T1.003** (Compromised Front-End broadly construed for the iOS push-notification infrastructure leg) + downstream **OAK-T4.004** / **OAK-T4.005** (drainer-class extraction once the malicious Snap was active in the user's wallet).
