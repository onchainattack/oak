...
**OAK Techniques observed:** OAK-T9.001 (Oracle Price Manipulation) — primary; OAK-T17.001 (Cross-Venue Arbitrage-Driven Price-Discovery Distortion) — adjacent cross-reference at the *cross-venue manipulation step* (FTX / AscendEX / Serum cross-venue thin-liquidity moves drove the price-discovery distortion that the Mango oracle subsequently consumed; the load-bearing loss-surface remains Mango's oracle, but the cross-venue manipulation step is structurally a T17.001 instantiation, preserved here so the corpus's T17.001 class anchor (`examples/2025-03-hyperliquid-jelly-self-liquidation-cross-venue.md`) cross-references the historical T17.001-adjacent record); OAK-T16.002 (Hostile-Vote Treasury Drain — settlement-vote second step that retained \~\$47M of the \~\$110M extracted; preserves the post-T9.001 governance-settlement chain at the operational sub-pattern level); OAK-T5.005 (Treasury-Management Exit — on-chain framing of the resulting DAO-treasury egress under the settlement vote); **OAK-T8.001** (Cluster-Reuse — Eisenberg's on-chain trading footprint across FTX/AscendEX/Serum venues was the load-bearing operator-attribution primitive; the wallet cluster's cross-venue activity pattern enabled attribution to a named individual, and the case is the canonical T8.001 worked example for the *self-identified-operator cluster-attribution* sub-pattern).
...
**Key teaching point:** Mango Markets is OAK's modern canonical T9.001 example because the technical mechanism, the regulatory record, and the legal-status evolution are all on the public record. Contributors writing other T9.001 examples should:
