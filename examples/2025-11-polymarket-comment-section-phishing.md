...
**OAK Techniques observed:** No clean v0.1 Technique. **Proposed v0.x candidate**: T4.x sub-class for *native-app social phishing on engagement-weighted platforms* — distinct from DM-spam phishing (T4 generic) because the distribution surface is a paid-for in-platform mechanic. The credential-capture stage is also distinctive: it is **not** an `eth_signTypedData` / Permit2 trap, it is a fake login + fake captcha that captures the user's email-magic-link credentials. See `TAXONOMY-GAPS.md`.
