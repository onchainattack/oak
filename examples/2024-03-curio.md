...
**OAK Techniques observed:** **OAK-T9.003** (Governance Attack — voting-power-via-storage-collision subclass) + **OAK-T9.002** (flash-loan precondition; loaned voting-power tokens to reach quorum) + **OAK-T9.004** (Access-Control Misconfiguration in the upstream MakerDAO-fork governance proxy) + **OAK-T16.001** (Vote Takeover via Flash-Loan — operational sub-pattern: flash-loaned voting-power tokens to reach quorum) + **OAK-T16.005** (Malicious Proposal Snowballing — canonical MakerDAO-fork-substrate sub-shape of the storage-collision class, paired with Audius 2022 and Tornado Cash 2023).
