...
**OAK Techniques observed:** **OAK-T9.014** (Protocol-Client Consensus Bug — the canonical anchor. The integer overflow in Bitcoin Core's `CheckTransaction()` function was a C++ reference-client arithmetic bug at the consensus layer, not a smart-contract vulnerability. The structural shape — validation bypass via unchecked integer arithmetic in consensus code — is the defining T9.014 pattern. See [`techniques/T9.014-protocol-client-consensus-bug.md`](../techniques/T9.014-protocol-client-consensus-bug.md)).
...
| 2010-08-15 ~17:30 | Block 74638 mined, containing transaction `1d40ca...` with two outputs of ~92.2 billion BTC each; output sum wraps around via integer overflow, bypassing value-creation check | T9.014 |
