...
**OAK Techniques observed:** none — pending reconciliation, not an absence of fit. **OAK-T7.008** (Stablecoin Issuer Freeze-Asymmetry Laundering) matches this file's mechanism closely, but tagging it would wire a direct contradiction into the relationship graph: `techniques/T7.008-*.md` states that **Circle froze** sanctioned USDC while **Tether did not**, and that laundering therefore shifted toward **USDT**. This file asserts the opposite polarity — that Tether freezes aggressively, Circle demonstrably does not, and operators therefore route through **USDC**. Both are in the corpus and both cannot be right as written. The likeliest reconciliation is time-scoping (a 2022 sanctions-order response versus a 2022–2026 general-illicit-funds posture), which would let both stand if each is stated precisely. Resolve the Technique page and this example against each other before tagging. (The prior field claimed bare OAK-T7, which resolves to no Technique.)
