...
**OAK Techniques observed:** none — no OAK Technique fits, and the case may fall outside OAK's scope. The mechanism (compromised Italian government email → forged subpoenas → abuse of X's Legal Request Portal to ban accounts and pull user data) produces no on-chain authority, no key or wallet compromise, and no crypto-asset loss. T15.004 requires a credential compromise whose downstream effect reaches an on-chain or operator-signing surface; this reaches platform moderation state only. Candidate T15 gap: platform legal-request-portal abuse via compromised government credentials. Note: this file's contributor note proposes the gap under T11, which is the wrong tactic — T11 is custody/signing.
