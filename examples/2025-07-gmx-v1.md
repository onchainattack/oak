...
**OAK Techniques observed:** **OAK-T11.013** (Legacy-Version Maintenance Attack Surface — the canonical anchor; GMX's operational decision to maintain V1 in production without backporting known vulnerability fixes converted the deprecated version's residual exploitable surface into a $42M loss. See [`techniques/T11.013-legacy-version-maintenance-attack-surface.md`](../techniques/T11.013-legacy-version-maintenance-attack-surface.md)). **OAK-T9.001** (Oracle / pricing manipulation — GLP-global-short-tracking accounting subclass; the specific vulnerability class exploited on V1) + **OAK-T9.005** (state-machine accounting flaw in V1's swap-and-stake transaction logic).
**Attribution:** **pseudonymous**; the attacker engaged with the bounty negotiation surface in pseudonymous form on-chain. No off-chain identification was published. The negotiation-acceptance behaviour is one of the cleaner behavioural signals for a pseudonymous-individual attacker classification rather than a state-actor / OAK-G01 classification (the OAK-G01 base rate is total laundering, not negotiation-acceptance).
