...
**OAK Techniques observed:** **OAK-T11.006** (Cold-storage Seed-phrase Exfiltration at Rest), **OAK-T11.006.001** (User-Initiated Plaintext-Equivalent Seed Storage) — primary; users stored seed phrases in LastPass vaults, the encrypted vaults were exfiltrated, and offline brute-force of master passwords yielded the seed material used to drain the corresponding wallets. The 2023-10-25 drain wave is the realised extraction leg, not a separate Technique. (The prior field claimed "OAK-T6 (Authorization / Key Compromise)" and "OAK-T5 (Asset Drain)" — neither name exists in the taxonomy: T6 is Defense Evasion and T5 is Value Extraction.) **PENDING DEDUP DECISION:** this file appears to be a strict subset of [`examples/2022-12-lastpass-vault-cohort.md`](2022-12-lastpass-vault-cohort.md) — its entire subject is one dated row in that file's timeline (2023-10-25, ~\$4.4M, ~25 victims), on which the two agree. Its cumulative "\$250M+" figure is unsourced and irreconcilable with the cohort file's sourced ~\$35M / ~150 victims plus the separately-tracked ~\$150M Larsen heist. Leaving both live means the corpus asserts two loss magnitudes and two exfiltration dates for one incident and double-counts it in T11.006's example count.
