...
**OAK Techniques observed:** **OAK-T11.011** (Multi-chain Key-store Co-location — the canonical anchor; the simultaneous multi-chain extraction pattern across ETH, TRX, BTTC, and other chains indicates co-located signing key material in Poloniex's hot-wallet infrastructure. See [`techniques/T11.011-multi-chain-key-store-co-location.md`](../techniques/T11.011-multi-chain-key-store-co-location.md)). **OAK-T4.001** (Private Key / Seed Phrase Compromise — the attacker compromised the co-located signing infrastructure). **OAK-T7.003** (Cross-Chain Bridge Laundering — the bulk of the extracted $120M was routed through cross-chain laundering rails into Lazarus-attributed laundering chains). **OAK-T15.003** (Operator Endpoint / Infrastructure Compromise — the proximate entry vector was intrusion into Poloniex's operator-side hot-wallet infrastructure).
