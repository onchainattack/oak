...
**OAK Techniques observed:** none — no Technique cleanly fits. The mechanism is advance-fee service fraud: fabricated Bybit demo-account PNL screenshots as the credibility layer, sale of a non-existent copy-trading service for \$250–\$500, a fake API key as a retention artifact, escalation to a \$20K demand, then block-on-payment. The file states the disqualifying fact itself — "a service-fraud model, not a wallet drain". T11.005.002 fails on two of its four requirements: there is no custodial deposit under management (victims pay a service fee) and no Ponzi payout structure (victims are blocked, nobody is paid). Candidate gap: fabricated-performance advance-fee service fraud with no custody and no Ponzi. **Flagged for adjudication:** if the \$20K "trade for them" escalation is read as the load-bearing leg rather than an escalation, T11.005.002 becomes tenable at low confidence.
