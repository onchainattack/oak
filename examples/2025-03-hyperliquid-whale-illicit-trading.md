...
**OAK Techniques observed:** none for the trading conduct — the file establishes no manipulation primitive. What is documented is outlier-leverage position-taking (3–5× platform average) that profited when the market moved; the file itself concedes the activity may be "aggressive trading" rather than manipulation, attribution is `inferred-weak`, and the subject denies involvement. No T17.* fits: no spoofing or cancel-flood (T17.003), no engineered liquidation cascade (T17.002), no cross-venue spread manufacture (T17.001), no oracle manipulation (T9.001). **OAK-T8.001** (Common-Funder Cluster Reuse) — attribution mechanism only; multiple accounts with correlated leverage patterns funded from one source. Candidate gap: outlier-leverage position-taking on a permissionless perp DEX with alleged information advantage, where the LP pool rather than an identifiable counterparty absorbs the loss.
