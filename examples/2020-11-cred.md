...
**OAK Techniques observed:** **OAK-T11.005** (Operator-Side / Fake-Platform Fraud — weak fit; Cred's failure was primarily a custodial-fraud / off-chain counterparty-risk insolvency with on-chain components. The operator-side misrepresentation of counterparty risk to depositors aligns with T11.005's operator-fraud framing, though the case's primary failure mode — off-chain counterparty default — is not cleanly captured by any v0.1 Technique. Flagged for TAXONOMY-GAPS.md). **OAK-T6.007** (Trust-Substrate Shift / Vendor-Promise Revocation — Cred's suspension of withdrawals and subsequent bankruptcy filing represents a trust-substrate shift: depositors who relied on Cred's yield promise and custody representations discovered the platform was insolvent).
...
**Key teaching point:** Cred November 2020 sits in OAK's corpus as a **deliberately weak-Technique-fit case included for the historical record and the taxonomy-gap evidence**. Its failure mode is off-chain custodial-fraud / counterparty-risk insolvency with on-chain components, not a primary on-chain Technique case. The OAK v0.1 Technique surface — focused on on-chain attack mechanics — does not cleanly capture the Cred failure, and contributors writing the case should not over-fit it to T9.x or T11.x. The honest framing is that *Cred is evidence for a taxonomy gap*: a future v0.x version of OAK might add an off-chain-counterparty-risk Technique class to capture this failure pattern, and Cred (alongside Celsius, Voyager, BlockFi, Genesis, FTX) would be the canonical worked examples for that class.
