...
4. **Proposal execution and drain.** The governance contract's proposal-execution function was callable immediately after proposal passage — no mandatory timelock delay existed (structurally parallel to T16.006 timelock-free execution). The attacker executed the proposal in the same transaction and drained the protocol's treasury of approximately $11M in ELEPHANT tokens and BNB.
...
| same transaction | Attacker swaps flash-loaned BNB for ELEPHANT governance tokens on PancakeSwap ELEPHANT/BNB pool; acquires controlling voting weight | **T16.001 execution (two-hop acquisition)** |
| same transaction | Attacker creates governance proposal granting treasury-drain authority to attacker-controlled address; votes with acquired ELEPHANT tokens; quorum met via flash-loan-funded voting weight | **T16.001 + T9.003 execution** |
...
| 2022-04-17 | Beanstalk Farms flash-loan governance attack on Ethereum (~$182M) — five days after Elephant Money, confirming the cross-chain T16.001 class maturity in April 2022 | **T16.001 class recurrence (Ethereum)** |
| 2022-05-08 | Fortress Protocol flash-loan governance attack on BNB Chain (~$3M) — 26 days after Elephant Money, confirming the BNB Chain T16.001 class maturity | **T16.001 class recurrence (BNB Chain)** |
...
- Cross-reference: T16.001 at `techniques/T16.001-flash-loan-governance-vote-manipulation.md`.
