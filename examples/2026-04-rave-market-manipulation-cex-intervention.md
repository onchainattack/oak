...
**OAK Techniques observed:** none — the file establishes the outflow but not the authority basis a Technique tag would need. A multisig linked to RAVE's initial distribution sent ~23M RAVE to two Bitget deposit addresses, with a -40% move within three hours; the entity that distributed the token to the market was the entity selling into it. T5.005 (Treasury-Management Exit) is the nearest home but its discriminator is a stated-purpose mismatch — outflows publicly framed as legitimate expenditure — and the file never establishes that the multisig is a treasury or that any purpose was stated. T5.006 is ruled out (no vesting cliff), T3.003 (no pump leg documented — only a crash), T3.006 (requires ≥4 simultaneous vectors; one is documented). The CEX deposit-address flow is a detection surface, not a mechanism. **Note:** the file's title claims market manipulation, but what its evidence establishes is an insider dump; the -95% first leg is unexplained.
