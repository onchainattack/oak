...
**OAK Techniques observed:** OAK-T11.002 (Wallet-Software Distribution Compromise) — primary on-chain manifestation; OAK-T15.002 (Supply-Chain / Vendor-Pipeline Compromise) — upstream entry vector at the wallet-vendor build-and-distribution pipeline, the canonical wallet-vendor sub-shape anchor; multi-rail downstream laundering including OAK-T7.001 (Mixer-Routed Hop, including Sinbad-class services), OAK-T7.002 (CEX Deposit-Address Layering — Garantex / OAK-G03 as one of several rails), and chain-hopping. The Garantex rail is *one* documented downstream surface, not the dominant or exclusive one.
