...
**Attribution:** **unattributed (protocol-internal bug — no external attacker).** The bug was introduced by the Yam Finance development team (a pseudonymous collective) during the rapid-deployment launch cycle. No hostile external actor exploited the bug for extraction. The bug was reported by community auditors and by DeFi-security researchers within ~24 hours of launch.
