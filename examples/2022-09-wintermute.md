...
**OAK Techniques observed:** **OAK-T11 broadly construed** (custody surface compromise via insufficient-entropy key generation). The case does **not** map cleanly to any v0.1 T11 sub-Technique: T11.001 (Third-Party Signing / Custody Vendor Compromise) does not fit — there was no signing-vendor compromise; T11.002 (Wallet Software Compromise) does not fit — the wallet software was not compromised, the off-line address-generation tool had a known cryptographic weakness. The case is a v0.x candidate for a new sub-Technique **T11.004 — Insufficient-Entropy Key Generation** (see Discussion). Downstream: T7 laundering applies (proceeds partially routed through mixing).
