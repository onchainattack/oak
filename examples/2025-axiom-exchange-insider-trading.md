...
**OAK Techniques observed:** none — platform-employee insider trading on customer order flow has no Technique at v0.x. Employees with legitimate internal-tool access queried customers' wallet activity and traded ahead of pending accumulation. Nothing was compromised (no T15.004, no T11 surface), no token was launched or promoted (no T3.003/T3.004), no supply was extracted through tokenomics (no T3.006), no order-book or price primitive was manipulated (no T17.*). The taxonomy's only insider-trading Technique, T9.006.004, is prediction-market-specific and requires the bettor to be a causal actor producing the underlying event — these employees front-ran an order, they did not cause the move. T5.004 does not fit either: its surface is the mempool and it requires a back-run leg, whereas the information here came from off-chain internal tooling reading future *intent*. Candidate gap, which the file argues itself: operator-employee insider trading on customer order flow.
