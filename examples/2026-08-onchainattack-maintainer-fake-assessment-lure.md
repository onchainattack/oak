...
**Forward candidate:** this entry is the field anchor for **OAK-T15.007** (*Fabricated Institutional Identity as Pretext Substrate*) — the credibility of the approach rested on an affiliation that was **manufactured for the campaign** rather than on any compromised account, which is what separates it from T15.006. Promotion to `emerging` still needs a second independent anchor carrying primary-source evidence of the identity itself (domain registration dates, account creation dates, employer confirmation).
