...
**OAK Techniques observed:** **OAK-T8.002** (Cross-Chain Operator Continuity) — primary; 8+ protocols across 6+ chains are individually coherent but jointly attributable only via cross-chain deployer-funding analysis. **OAK-T8.001** (Common-Funder Cluster Reuse) — each new deployer wallet was funded from wallets tied to previous rugs; the funding source is the cluster identifier. **OAK-T7.001** (Mixer-Routed Hop) / **OAK-T7.003** (Cross-Chain Bridge Laundering) — proceeds laundered, then recycled as seed liquidity for the next deployment. **Not mapped:** the extraction event itself. The file never describes *how* TVL was withdrawn (no backdoor, admin key, or LP mechanism), and these are lending/yield protocols where the drained asset is depositor TVL rather than LP depth, so T5.001 is not supportable.
