...
**Attribution:** **unattributed (no adversarial threat-actor — consensus-client bug).** The finality loss was caused by a bug in the Prysm and Teku consensus clients' attestation-processing logic, not by an attacker. The Ethereum Foundation, client teams, and community researchers published detailed post-mortems. No adversarial attribution applies; the incident is a protocol-layer operational event.
