...
**Key teaching point:** **This is OAK's canonical example of the embedded-wallet identity-provider compromise class — an attack surface that collapses "non-custodial wallet" security to "email account security."** When a dApp uses Magic Labs / Privy / Dynamic / Web3Auth as its authentication layer, the user's wallet is only as secure as their email account. The attacker does not need to phish a seed phrase, compromise a hardware wallet, or bypass a smart-contract security mechanism — they only need to capture the email magic-link code. This class is increasingly important as more crypto applications adopt email/social-login flows, and the attack surface is invisible to every wallet-side defense (hardware wallet display, Permit2 warnings, approve-revoke tooling) because none of those defenses are in the loop.
