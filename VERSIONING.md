...
| 0.9 | 2026-09 | **Additive: the relationship graph is emitted in full.** `tools/oak.json` previously emitted only `mitigates` (M→T) and two `uses` forms (S→T, G→S) — 478 edges of roughly 2,700 — and omitted the corpus's primary relation entirely. Four edge types added: **`demonstrates`** (worked example → Technique, and → Tactic where an example maps to a bare Tactic; 1,912 edges), **`belongs-to`** (Technique → parent Tactic; 199), **`subtechnique-of`** (sub-Technique → parent Technique; 19), **`attributed-to`** (worked example → Threat Actor, read from each actor's `## Observed Examples` section; 71). Total 478 → 2,679. The STIX bundle gains **`x-oak-incident`** SDOs for all 713 worked examples — previously absent, so the export described a taxonomy with no evidence base — and carries every new edge as an SRO (770 → 3,684 objects). Custom SDO rather than the STIX 2.1 `incident` type, which is a reserved stub with no defined properties; matches the existing `x-oak-tactic` / `x-oak-data-source` convention. **Non-breaking:** no existing edge changed type, shape or semantic, no ID was renamed, and every previously emitted relationship is still emitted identically — per the additive rule and the explicit "Adding new relationships between existing objects" case above. Consumers pinned to the 0.8 shape keep resolving; consumers that enumerate relationship types will see four new values. Six example Technique references point at TAXONOMY-GAPS candidate IDs with no object to link to yet; `export_json.py` reports them rather than dropping them silently. |
