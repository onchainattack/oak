...
**Maturity:** emerging
**Chains:** chain-agnostic (anchors on Solana → 18 destination chains, XRP Ledger → Coreum, and Tron)
**First documented:** 2026-07-17 (Across / Risk Labs on Solana); promoted on the second independent anchor, 2026-08-09 (Coreum–XRPL bridge); third anchor 2026-09-12 (Chainflip, Tron)
...
`VERSIONING.md` puts the `emerging` → `stable` bar at **three anchored worked examples plus multi-source forensic attestation**. With Chainflip (2026-09-12) the **anchor count is met**. The **attestation leg is not**: Across and Coreum each carry independent third-party forensic analysis, while the Chainflip account rests on the protocol's own disclosure and press reporting of it, with its full technical report outstanding at time of writing. **Promotion is deferred rather than declined** — the trigger is Chainflip's post-mortem or an independent analysis of the Tron path. A fourth anchor from the **intents / solver population**, where the observer's authority is largest and the review gap widest, would settle it regardless.
