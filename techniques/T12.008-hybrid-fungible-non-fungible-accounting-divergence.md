...
- **Evaluated and rejected as a second anchor (2026-09-12): MINER / ERC-X (2024-02, 168.8 ETH).** An ERC-404-family contract, and a real exploit — but the defect was a **self-transfer read-modify-write ordering error on the fungible ledger** (`_update` missing a `from != to` check and writing a cached recipient balance). No identifier was aliased, no ownership predicate was bypassed, and **the identical code in a plain ERC-20 carries the identical bug**, which is the disqualifier this page's scope paragraph names. A hybrid-standard contract that had a bug is not a hybrid-standard bug. Documented at [`examples/2024-02-miner-erc-x-self-transfer-cached-balance-double-spend.md`](../examples/2024-02-miner-erc-x-self-transfer-cached-balance-double-spend.md) under T9.004 + T9.005. **What would qualify:** fungible supply diverging from escrowed count, a redemption paying out an NFT the position did not entitle, or an identifier resolving differently in an authorisation check than in accounting.
