...
- **Protocol-client consensus bug (proposed T9.x).** C++ reference-client integer-overflow / input-validation-bypass bugs that operate at the consensus layer rather than the smart-contract layer. The canonical anchor is the Bitcoin value overflow bug (2010-08-15, block 74638, CVE-2010-5139) — a C++ integer overflow in Bitcoin Core's `CheckTransaction()` allowed creation of 184B BTC via unchecked output-value summation. OAK v0.1's T9 surface is focused on smart-contract exploits; protocol-client bugs in C++/Rust reference implementations are not captured. The structural distinction is material: smart-contract bugs are patched via governance/proxy-upgrade; protocol-client bugs require miner/validator-coordinated chain reorganisation or emergency hard fork. See [`examples/2010-08-bitcoin-value-overflow-bug.md`](../examples/2010-08-bitcoin-value-overflow-bug.md).
...
- **Off-chain counterparty-risk / yield-without-due-diligence failure (proposed T11.x).** Custodial-yield platforms that fail because the operator extended unsecured credit to an offshore counterparty with insufficient due diligence, then absorbed the counterparty's default as a customer-facing solvency event. The failure mode is off-chain (operator credit decision → counterparty default → customer-asset shortfall), with on-chain components (deposit/transfer traceability) but no on-chain exploit. The canonical anchor is Cred Inc. (2020-11-07, Chapter 11, ~$140M+), with structural descendants at Celsius (~$1.2B+), Voyager (~$1B+), BlockFi (~$1B+), Genesis (~$2B+), and FTX (~$8B+). T11.005 (Operator-Side Fake-Platform Fraud) is the closest v0.1 Technique but does not capture the structural shape — the operator did not create a fake platform; the operator made real but disastrously under-diversified counterparty credit decisions. Flagged by [`examples/2020-11-cred.md`](../examples/2020-11-cred.md) as a taxonomy-gap evidence case.
