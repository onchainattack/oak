...
- **STIX 2.1 export** at [`tools/oak-stix.json`](./tools/oak-stix.json) — full STIX 2.1 bundle (601 objects: 14 x-mitre-tactic + 62 attack-pattern + 40 course-of-action + 28 malware + 12 tool + 18 intrusion-set + 12 x-mitre-data-source + 415 relationships) for interop with TIPs, SIEMs, and other ATT&CK-aware tooling. Regenerated by `python tools/export_stix.py` from the markdown sources via `tools/oak.json`.
...
├── PRIOR-ART.md             # positioning vs MITRE ATT&CK, OWASP SC Top 10, academic SoK
