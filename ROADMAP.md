...
- ~~**STIX 2.1 export**~~ — **delivered**. `tools/export_stix.py` emits `tools/oak-stix.json` (597 STIX objects: x-mitre-tactic + attack-pattern + course-of-action + malware/tool + intrusion-set + x-mitre-data-source + relationship SROs). UUIDs derived deterministically via uuid5 from a pinned OAK namespace so STIX IDs stay stable across regenerations.
